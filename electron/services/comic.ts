...
/** 支持的容器格式：CBZ=zip、CBT=tar */
export type ArchiveKind = 'zip' | 'tar' | 'unknown';
...
let cache: { kind: 'zip' | 'tar'; path: string; mtimeMs: number; zip?: JSZip; tar?: TarEntry[]; buffer?: Buffer } | null =
  null;

async function loadArchive(archivePath: string): Promise<NonNullable<typeof cache>> {
  const stat = fs.statSync(archivePath);
  if (cache && cache.path === archivePath && cache.mtimeMs === stat.mtimeMs) return cache;

  const buffer = fs.readFileSync(archivePath);
  const kind = detectArchiveKind(buffer.subarray(0, 512));
  if (kind === 'zip') {
    cache = { kind, path: archivePath, mtimeMs: stat.mtimeMs, zip: await JSZip.loadAsync(buffer) };
  } else if (kind === 'tar') {
    cache = { kind, path: archivePath, mtimeMs: stat.mtimeMs, tar: readTarEntries(buffer), buffer };
  } else {
    throw new Error('无法识别的压缩包格式（支持 CBZ / CBT）');
  }
  return cache;
}
...
export async function listComicPages(archivePath: string): Promise<string[]> {
  ...
  return archive.tar!.map(e => e.name).filter(isComicPage).sort(naturalCompare);
}
...
export async function readComicPage(
  ...
  const entry = archive.tar!.find(e => e.name === name);
